# Main file for recovering stiff Van der Pol oscillator
import numpy as np
import scipy
...
if os.path.isfile(filepath):
    ...
    for i in range(int(nmc)):
        # draw uniformly perturbed initial condition around [2, 0]
        x0 = np.array([2, 0]) + 0.05 * np.random.randn(2)
        exact_path = scipy.integrate.odeint(systems.van_der_pol, x0, tgrid)
...
model = ResNet(n_mem, model=model)
model_path = "./data/vdp/trained_model"
